...
## Components, schema v2, import resolution (v0.3.0)

- **Upgrade test seam** (`db.test.ts`): build the old schema directly — `openDB('ui-rover', 1, { upgrade })` under fake-indexeddb, seed rows, close — then call the module's `openDb()` and assert entries survive and `by-parent` works. A forced upgrade failure must yield `storageFallbackReason() === 'upgrade-failed'`.
- **URL-less entries**: two entries without `url` must coexist in IndexedDB *and* in the forced in-memory fallback (the fallback once compared `undefined === undefined`).
- **Component integrity**: `createEntry` rejects a missing/component parent, a foreign `parentImageId`, a wrong `sourceImageId`, two images; cascade delete is atomic (a failing child-image delete removes nothing); `listChildren` order.
- **Import resolution** (`export.test.ts`): `resolveImport` is pure — collisions first, then the parent-id set that exists after the operation (Merge: existing ∪ accepted; Replace: accepted only); orphan when the parent is absent, skipped for a URL collision, or `parentImageId` is foreign; parents-first ordering; both modes return `{ imported, skipped, orphaned }`.
- **Component UI**: `ComponentCapture.test.tsx` stubs `CropTool` (calls `onChange(rect)` then `onConfirm()`) and mocks `cropComponent`; the editable-chips persistence test uses the real 300 ms debounce inside `waitFor` (fake timers + fake-indexeddb's `setImmediate` made flush ordering unreliable).
- **Driving `CropTool` from a real-browser script**: dispatch pointer events on the `[role=group][aria-label="Crop region"]` overlay, stub `Element.prototype.setPointerCapture` (the native one throws for synthetic pointer ids), and **yield a frame between events** — back-to-back dispatches land before React commits the drag state.
...
v0.3.0 manual checks: (1) open the app on a v0.2.0 database — all entries still listed, no storage banner; (2) save a screenshot with no URL — "Untitled capture", no source link, no platform badge; (3) on a design entry capture two components (e.g. the palette card → *Card*, the "SAVE FOR LATER" pill → *Button*) — both appear in the Components strip as WebP crops; (4) Library: kind *Component* and component type *Button* filter correctly; (5) a component page shows the parent link, editable chips, no nested capture, and Extract palette reads it (`paletteSource: 'ocr'` on the card) without touching `parentImageId`/`sourceRect`; (6) deleting the parent removes its components.

## Current Counts

Last recorded via `npx vitest run` (2026-09-17): **46 test files, 412 tests, all passing.** Re-run the command above to refresh this before relying on it for a release decision.
