...
You are now in **implementation mode** for **ui_rover**.
...
## Model Routing — TRIP-2

Work is routed by **batch**, not wholesale. Three lanes:

| Batch character | Goes to | How |
|---|---|---|
| Hardest — novel core logic, security, data integrity, cross-cutting | **Sol** (senior builder) | Codex bridge, escalated: `CODEX_MODEL=gpt-5.6-sol` |
| Ordinary — real work, follows existing patterns, needs judgment | **Opus** (senior reviewer, building here) | **In-session, directly. Not delegated.** |
| Parallel low-risk — mechanical, repetitive, independently verifiable | **Luna** (fast assistant) | Codex bridge, default lane |

**Codex is the outside reviewer, so Codex-implemented work always comes back for review** — Sol's and Luna's batches both get the delta review in §3. Opus's own batches get the same delta review discipline, but from the author, so lean harder on the testing gate and the final Codex code review for those.

The judgment call is *what kind of work is this*, not *how big is it*. A three-line change to an auth check is Sol's. A four-hundred-line mechanical rename is Luna's.

---

## Implementation Phase

Ordinary batches you implement yourself. Hard and parallel-low-risk batches go to Codex via the `codex-implement` skill. (Trivial unplanned changes of a few lines are always done directly.)
...
**Then tag each batch with its lane** (Sol / Opus / Luna, per Model Routing above) before delegating anything. Write the lane assignment down — it is what you check the returned diff against, and a batch that came back wrong is often a batch routed wrong.
...
**Escalate a hard batch to Sol** by setting `CODEX_MODEL` on that call — one variable flips model, effort, and tier to the hard lane. No separate skill, no separate thread:

```bash
CODEX_MODEL=gpt-5.6-sol bash ~/.claude/skills/codex-implement/scripts/start.sh \
    --prompt-file ~/.claude/skills/codex-implement/prompts/implement.tpl \
    <plan-path> "Implement only: <hard batch checkboxes>"
```

The same `CODEX_MODEL=gpt-5.6-sol` prefix works on the resume call below, so lanes can alternate batch to batch within one thread. Every invocation prints its resolved `lane/model/effort/tier` before it runs — check that line matches the lane you assigned.
...
npx tsc -b --noEmit 2>&1 | tee /tmp/_trip2-typecheck.txt   # the -b is mandatory; bare `tsc --noEmit` is vacuous in this Vite project-references setup
npm run build 2>&1 | tee /tmp/_trip2-build.txt              # production build via tsc -b && vite build
```
...
Only the files/areas the change touched — never the full suite by default. Vitest is planned, not yet installed — until the plan that adds it lands, report this gate as SKIPPED, never as passed.

### 3. Integration impact check

ui_rover has no E2E suite. If the change touches `features/capture` (URL parsing / paste-drop) or `lib/db.ts` (IndexedDB), run `npm run dev` and exercise the flow in the browser once: paste a real Instagram or Threads URL + a screenshot, confirm the entry appears in the library after a reload. Docs-only changes skip this.
